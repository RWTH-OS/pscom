...
int pscom_parse_socket_ondemand_str(const char *socket_str, int *nodeid, int *portno, char (*name)[8])
{
	...
	if (pscom_ascii_to_sockaddr_in(host, port, "tcp", &sock) < 0) goto err_to_sock;
...
{
	return pscom_socket_ondemand_str(pscom_get_nodeid(), socket->listen_portno, socket->local_con_info.name);
}
...
{
	static char buf[sizeof("(xxx.xxx.xxx.xxx,pidxxx,0x_xblast_xblast__,XXXXXXXXXXXXXXXX)_____")];
	snprintf(buf, sizeof(buf), "("INET_ADDR_FORMAT",%d,%p,%.8s)",
		 INET_ADDR_SPLIT(con_info->node_id),
		 con_info->pid,
		 con_info->id,
		 con_info->name);
	return buf;
...
const char *pscom_req_state_str(pscom_req_state_t state)
{
	...
	const struct names {
		...
		{ PSCOM_REQ_STATE_SEND_REQUEST, "send" },
