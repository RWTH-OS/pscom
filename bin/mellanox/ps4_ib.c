...
static
int psib_recvlook(con_info_t *con_info, char **buf)
{
...
int nbread(int timeout/*us*/,int fd,void *buf,size_t count){
    fd_set rfds;
    struct timeval tv;
    /* Watch fd to see when it has input. */
    FD_ZERO(&rfds);
    FD_SET(fd, &rfds);
    /* Wait up to timeout us. */
    tv.tv_sec = 0;
    tv.tv_usec = timeout;
...
do_test(void)
{
    ...
    sleep(2);
#define STR_PTR_LEN(str) str , sizeof(str) + 1
    psib_send(&con, STR_PTR_LEN("HalloWorld Message 1"));
    psib_send(&con, STR_PTR_LEN("HalloWorld Message 2"));
    psib_send(&con, STR_PTR_LEN("HalloWorld Message 3"));
    psib_send(&con, STR_PTR_LEN("HalloWorld Message 4"));
    psib_send(&con, STR_PTR_LEN("HalloWorld Message 5"));
    psib_send(&con, STR_PTR_LEN("HalloWorld Message 6"));
...
//    }
	...
	psib_poll(&default_hca, 1);
	len = psib_recvlook(&con, &buf);

	if (len >= 0) {
	    buf[len] = 0;
	    printf("Receive %d bytes : %s\n", len, len > 0 ? buf : "nix");
	    psib_recvdone(&con);
	    printf("....... recv: %d tosend: %d send: %d\n",
		   con.n_recv_toks, con.n_tosend_toks, con.n_send_toks);
	}
	{
	    char msg[100];
	    int len;
	    len = nbread(0, 0, msg, 100);
	    if (len > 0) {
		len--;
		psib_send(&con, msg, len);
		printf("Send %d bytes....... recv: %d tosend: %d send: %d\n",
		       len, con.n_recv_toks, con.n_tosend_toks, con.n_send_toks);
	    }
	}
    }
    return;
